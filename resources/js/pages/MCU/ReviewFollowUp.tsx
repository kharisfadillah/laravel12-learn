...
export default function FollowUp({ mcu }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Review Tindak Lanjut', href: `/mcu/${mcu.id}/follow-up` },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/mcu/${mcu.id}/review-follow-up`);
    };
            ...
            <div className="max-w-7xl p-6">
                ...
                <Tabs defaultValue="awal" className="mt-4 w-full">
                    ...
                    <TabsContent value="lanjut">
                        <Card>
                            <CardContent className="grid grid-flow-row grid-cols-4 gap-2 px-3 py-0">
                                <RecordItem label="Tanggal MCU Tindak Lanjut" value={mcu.followup?.mcu_date ?? '-'} />
                                <RecordItem label="Provider MCU Tindak Lanjut" value={mcu.followup?.provider?.name ?? ''} />
                            </CardContent>
                        </Card>
                        <Card className="mt-3 gap-2 py-3">
                            <CardHeader className="px-3">
                                <CardTitle>Dokumen MCU Tindak Lanjut</CardTitle>
                            </CardHeader>
                            <CardContent className="px-3 py-0">
                                <AttachmentList attachments={mcu.followup?.attachments ?? []} />
                            </CardContent>
                        </Card>
                        <Card className="mt-4 gap-3 py-3">
                            <CardHeader className="flex flex-row justify-between px-3">
                            ...
                            <CardContent className="gap-2 px-0 py-0">
                                <Table>
                                    ...
                                    <TableBody>
                                        {(mcu.followup?.items.length ?? 0) > 0 ? (
                                            mcu.followup?.items.map((item, index) => {
                                                // console.log(item.ranges);
                                                return (
                                                    <TableRow key={index}>
                                                        <TableCell className="px-3 py-1">{item.category?.name}</TableCell>
                        ...
                        <form onSubmit={handleSubmit} className="mt-3">
                            <Card className="mt-4 gap-2 py-3">
                                <CardHeader className="px-3">
                                    <CardTitle>Review MCU Tindak Lanjut</CardTitle>
                                </CardHeader>
                                <CardContent className="grid grid-flow-row grid-cols-3 gap-2 px-3 py-0">
                                    <div>
